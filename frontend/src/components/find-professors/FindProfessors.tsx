...
const FindProfessors = () => {
  ...
  const handleMultiFilterChange = useCallback(
    (filterName: "state" | "school", values: string[]) => {
      setSelectedFilters((prev) => {
        ...
        if (filterName === "state") delete next.school; // وابستگی: تغییر state → school ریست
        return next;
      });
    },
    []
  ...
  const isApplyEnabled = useMemo(() => {
    return Boolean(selectedFilters.country && selectedFilters.areaOfStudy);
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow p-4 mb-6">
          ...
          <div className="flex flex-wrap items-center gap-2 md:gap-4">
            ...
            <FilterDropdown
              ...
              onSelect={(v) => {
                setSelectedFilters((prev) => {
                  const next = { ...prev, areaOfStudy: v, page: 1 };
                  delete next.program; // تغییر area → program پاک شود
                  return next;
                });
              }}
              fixedWidthClass={FILTER_WIDTH}
            ...
            <FilterDropdown
              ...
              onSelect={(v) =>
                setSelectedFilters((prev) => ({ ...prev, program: v, page: 1 }))
              }
              disabled={
                loadingPrograms ||
                !selectedFilters.areaOfStudy ||
